    ...
    <form class="mt-5">
      <div class="row">
        <div class="col-12">
          ...
          <input type="number" name="amount" id="amount" v-model="data.orderQty" min="0" max="50000" class="form-control">
        </div>
...
export default {
  ...
  methods: {
    async order(name, type) {
      ...
      if (this.data.orderQty > 0) {
        await this.$makeRequest('POST', 'order', this.data)
        .then(data => {
          const {side, ordStatus, orderQty, currency, price} = data
          if (ordStatus === "Filled") {
            alert(`${side} ${orderQty} ${name} successfully for ${price} ${currency}!`)
          }
        })
        .catch(err => {
          alert(err.message)
